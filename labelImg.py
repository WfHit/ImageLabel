...
import resources
# Add internal libs
from libs.autoLabel import AutoLabel
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        self.LABEL_LIST = [str(i) for i in range(len(self.cmap))]
        ...
        autoLabel = action('&Auto Label', self.func_autoLabel,
                           'Ctrl+L', 'autolabel', u'Label image with CNN')
        
        checkLabel = action('&Check Label', self.func_checkLabel,
                           'Ctrl+K', 'labelcheck', u'Check Labeled image with CNN')

        convertLabel = action('&Convert Label', self.func_convertLabel,
                           'Ctrl+R', 'labelconvert', u'Convert Label to cvs')
        ...
        addActions( self.menus.view, 
                   (self.autoSaving,
                    ...
                    fitWindow, fitWidth) )                    
        addActions( self.menus.autoLabel, (autoLabel, checkLabel, convertLabel) )
    ...
    def saveLabels(self, annotationFilePath):
        ...
        try:
            if self.usingPascalVocFormat is True:
                ...
                self.labelFile.saveYoloFormat(annotationFilePath, 
                                              shapes, 
                                              self.filePath, 
                                              self.imageData, 
                                              self.labelHist,
    ...
    def openNextImg(self, _value=False):
        ...
        if self.autolabelrun :
            if self.cnn_running :
                print("cnn_running")
                return
            self.image_cnn = Image.open(filename)
            print(filename)
    ...
    def img_loop(self):
        #image_path = '../team_work/CapstoneImages/Luciano_Images/images/light_images_all'
    ...
    def func_checkLabel(self):
        if not self.check_label : 
            self.annotations = []
            self.annotation_path = self.dirname+"/images_label.cvs"
            print(self.annotation_path)
            with open(self.annotation_path, 'r') as annofile:
    ...
    def get_vertical_color(self, bimg):
        h = bimg.shape[0]
        w = bimg.shape[1]
        dl = int(h/3)
        dx0 = int(h/9)
    ...
    def get_horizontal_color(self,bimg):
        h = bimg.shape[0]
        w = bimg.shape[1]
        dl = int(w/3)
        dx0 = int(w/9)
    ...
    def get_color(self, img):
        if img.shape[0] > img.shape[1]:
            return self.get_vertical_color(img)
        else:
            return self.get_horizontal_color(img)
            
    def auto_lable(self):
        #GRAPH_FILE = '../team_work/CapstoneImages/faster_rcnn_inception_resnet_v2_atrous_coco_2018_01_28/frozen_inference_graph.pb'
        #GRAPH_FILE="ssd_mobilenet_v1_coco_2017_11_17/frozen_inference_graph.pb"
        GRAPH_FILE = self.cnn_dir+'/frozen_inference_graph.pb'
        print(GRAPH_FILE)
